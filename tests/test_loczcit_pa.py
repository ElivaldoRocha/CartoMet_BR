...
class TestBuildRaster:
    ...
    def test_clear_sky_excluded_without_convergence(self):
        ...
        nf = NF(
            tsm_n=np.ones(shape),
            conv_n=np.ones(shape),
            olr_n_inv=np.ones(shape),
            olr_abs=np.full(shape, 300.0),  # tudo céu limpo (>240)
            lons=np.linspace(-55, 15, 10),
    ...
    def test_clear_sky_rescued_by_convergence_is_cinematica(self):
        ...
        nf = NF(
            ...
            lons=np.linspace(-55, 15, 12),
            lats=np.linspace(-15, 15, 12),
            conv_abs=conv,
            lsm=np.zeros(shape),  # tudo oceano
        )
...
class TestLoczcitResult:
    def test_creation(self):
        r = LoczcitResult(
            ...
            lons=np.zeros(5),
            lats=np.zeros(5),
            valid_time="2026-05-31T12:00",
            base_time="00Z 31/05/2026",
        )
