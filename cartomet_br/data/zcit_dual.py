...
def _column_modes(
    weight_col: np.ndarray, lats: np.ndarray, grid_deg: float,
...
def _consolidate_branches(close: np.ndarray, single: np.ndarray,
                          north_c: np.ndarray, south_c: np.ndarray):
...
def detect_zcit_axis_dual(
    ...
    trunk_s = smooth_axis(lons, reject_outliers_iqr(trunk, k=iqr_k, window=iqr_window),
                          frac=lowess_frac, max_gap=max_gap)
    north_s = smooth_axis(lons, reject_outliers_iqr(north, k=iqr_k, window=iqr_window),
                          frac=lowess_frac, max_gap=max_gap)
    south_s = smooth_axis(lons, reject_outliers_iqr(south, k=iqr_k, window=iqr_window),
                          frac=lowess_frac, max_gap=max_gap)
    ...
    return ZcitAxisDual(
        lons=np.asarray(lons), lat_north=lat_north_s, lat_south=lat_south_s,
        n_modes=n_modes, is_double=bool(close.any()),
        double_fraction=double_fraction, coverage=coverage, envelope_mask=mask,
        meta={
            "t_env": t_env, "branch_min_run": branch_min_run,
            "min_sep_deg": min_sep_deg, "min_prom_frac": min_prom_frac,
            "grid_deg": grid_deg, "coherence": coherence,
